...
17 のセマンティック操作で、AI がファイルを直接読まずに必要な意味単位だけを取得する。
構造アクセスは全て Python が担い、常に `{ prompt, value }` を返す（prompt=value の読み方の
指針）。ブロック単位の操作は対象 block の x-prompt-query を schema から動的算出し、
メタ/集約系操作（get_meta/scan/index_scan/find_all）は動的に導出できないため
操作の性質に基づく固定文言を使う（値を返すすべての操作で読み方の指針を省略しない）。
ディレクトリ横断のindex集約（index_scan_documents）はuc-query-document-collection側が担う。
