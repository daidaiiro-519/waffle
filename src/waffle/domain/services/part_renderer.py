...
class MalformedContentError(Exception):
    """x-renderが配列を期待する部品（list/table/section/sequence/statediagram/
    architecture/flowchart）に対し、対応するcontent値が配列でないときに送出する。
    fillは値の型を検証しない設計（validateの責務）のため、fill後にvalidateを経ずに
    renderすると、配列を期待する箇所に文字列等が残ったまま渡ってくることがある。
    その場合に1文字ずつの箇条書き等の意味不明な出力を静かに生成する代わりに、
    ここで明確なエラーとして検出する。"""


_ARRAY_PART_KINDS = {"list", "table", "section", "sequence", "statediagram", "architecture", "flowchart"}
...
def render_part(part: dict, data: dict, level: int) -> str:
    ...
    if "from" in part and src and kind in _ARRAY_PART_KINDS and not isinstance(src, list):
        raise MalformedContentError(
            f"'{part['from']}' は配列であるべきですが {type(src).__name__} でした（as={kind}）。"
            "fill後にvalidateを経ずにrenderした可能性があります。"
        )
