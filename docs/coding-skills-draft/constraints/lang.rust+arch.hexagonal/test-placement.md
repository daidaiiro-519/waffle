...
| `adapter::inbound` | `tests/` | 外部の形式から業務操作へ渡るまで | 公開する境界から呼ぶ |
| `adapter::outbound` | `tests/` | 出力ポートの実装が契約どおりに振る舞うか | 公開する境界から呼ぶ |

## 偽物の置き場所

| 偽物 | 置き場所 | 作り方 | 置いてはならない場所 |
|---|---|---|---|
| 出力ポートの偽物 | `application::ports` の隣（`#[cfg(test)]` の中） | ポートのトレイトを実装した最小の型 | `adapter`。外側の実装とテストの偽物が混ざる |
| 入力の見本 | `tests/fixtures/` | 実物の形式そのまま | 内側。内側が外部の形式を知ることになる |
