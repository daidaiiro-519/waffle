...
11. ~~論点3（Hooks/permission設定）は、実運用でしばらく使ってみてから`update-config`スキルで実際に設定する~~ → `docs/brainstorm/brainstorm-waffle-hooks.md`に合流（2026-07-22）。実運用で document.json直接編集・spec-first違反の実例が複数回観測されたため、Hook候補の洗い出し・実現可能性シミュレーション・Go/No-Go判断まで完了。実装（候補5から着手予定）は同ブレスト側で追跡する
