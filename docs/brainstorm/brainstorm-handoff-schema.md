...
- Handoffが単一の形で済むのか、CodingSchemaのような`kind` discriminatorで
  複数種類を持つべきか（上記追記参照）。kindを分けるとしても、その分類軸が
  「技術調査／要約転記／方針文書」のような直感的な3分類で正しいかは未検証
  （実例3件だけからの帰納であり、他にもkindが存在する可能性がある）
