...
## 追記: 集約・業務サービスの所属先はsubdomainではなくbounded-context（2026-07-22）

`check-aggregate-class-drift`実装後の実データ調査で、Document集約
（`agg-document`）が`sd-document-management`と`sd-validation`という
**2つの異なるsubdomain**に属するusecaseの両方から`aggregateRef`で参照されて
いることを発見した。「サブドメインに複数の集約が存在しないのか」という問いを
きっかけに、この関係がDDD原則として正しいかをddd-advisorに相談した。

### 集約の所属先（ddd-advisor判定）

以下、要点のみ記す（全文は本セッションのやり取りに残る）。

- 「1つの集約が複数の異なるsubdomainに属するusecaseから操作される」構造は
  **DDD原則として正しい**。DDDが問うのは「同一データへの一連の業務操作が
  複数の**境界づけられたコンテキスト**に分散していないか」であり、サブ
  ドメインの単位ではない。サブドメインとコンテキストを1対1に固定すること
  自体がアンチパターン
- **集約が帰属するのは境界づけられたコンテキストであり、サブドメインでは
  ない**。サブドメインは「事業活動の分類（投資判断の道具）」であって、
  実装物の所属先を決める概念ではないため、agg-schema.json等の集約specが
...
同じ論理を業務サービスにも当てはめて確認した。実際に使用箇所を調べたところ、
`schema_discriminator.py`（「discriminatorキー抽出」、`sd-document-management`
の業務サービスとして宣言）が、`render_document.py`/`scaffold_document.py`
（`sd-document-management`）だけでなく**`check_schema_version_drift.py`
（`sd-reconciliation`）からも使われている**ことを確認した。集約と全く同じ
構図で、宣言先subdomainに縛られず複数subdomainのusecaseから利用されている。

Eric Evansの業務サービスの定義（特定のEntity/Value Objectに自然に属さない
操作であり、しばしば**複数の集約にまたがる**、`domain-model.md`にも同じ
記述がある）に照らすと、業務サービスは集約より広い範囲を扱うのが定義上の
常態であり、集約以上に単一のsubdomainへ押し込める理由がない。

**確定した結論**: 業務サービスが所属すべきは境界づけられたコンテキスト
（`bc-waffle`）であり、現在`DomainServicesBlock`が`SubdomainContent`配下に
定義されている構造（`x-prompt-query`: 「集約に属さないためここ(subdomain)に
...
### 記録場所についての検討（2026-07-22）

この発見を、既存の`ImplementationGuidanceBlock`（"詳細設計ガイド"、subdomain
contentの必須ブロック）に書くべきかを検討したが、そちらは「この業務領域の
実装の厚み（中核=ドメインモデルで厚く／一般=既製ライブラリを薄く包む／
補完=薄いトランザクションスクリプト、`CodingTemplate`の`subdomainRichness`
の宣言元）」という狭いスコープの分類ラベル用ブロックであり、今回のような
スキーマ構造上の発見を書く場所ではないと判断した。ユーザーから「今後作成する
詳細設計ガイド的なもの」という発言もあったが、これは`brainstorm-handoff-
schema.md`が扱う「進行中の検討・意思決定を記録するdocumentジャンルがWaffle
に無い」という論点と関連しうる別の検討事項であり、今回は本ブレストへの記録に
留めた。
...
`brainstorm-cli-native-document-and-schema-editing.md`の最後に残っていた項目
（「実運用でしばらく使ってみてから`update-config`スキルで実際に設定する」）が
本ブレストに合流する形で、以下3点の未検討事項に決着をつけた。

### 用途1と用途2は同一機能か別usecaseか

**そもそも同じ種類の成果物ではない**、という整理で決着。

- 用途1（候補2・4）: `check-usecase-class-drift`/`check-aggregate-class-drift`/
  `check-operation-drift`という**既存のWaffle usecaseが既にある**。Hooksが
  新規に提供するのはトリガーだけで、新しいusecase自体は不要——既存usecaseを
  PostToolUseから叩く薄いラッパースクリプト＋`.claude/settings.json`の設定
  だけで完結する
- 用途2（候補5・6）: パスパターンマッチ（候補5）・spec存在確認（候補6）
  という、Waffleの「engine操作」と呼べるほどの複雑さを持たない、Claude Code
  固有のガードレールスクリプト。CLI/MCPとして公開する意味も薄い（他ツールから
  「document.json直接編集を禁止して」と呼び出すユースケースが無い）

「同一機能に統合すべきか」という問い自体が的外れで、**用途1はゼロ実装（既存
usecase＋Hook設定のみ）、用途2は`.claude/hooks/`直下の小さな独立スクリプト**
という、性質の違う2つの成果物として扱うことにした。

### Waffle自身がHooks機構を持つべきか

**持たない**、で決着。マルチツール対応ブレスト（`brainstorm-multitool-loop-
goal.md`）と同じevidence-based-scope原則——Waffleが独自のHooks/イベントバスを
持つ動機は「Claude Code以外のツールでも同じ仕組みを使いたい」という実例が
あって初めて生まれるが、今のところWaffleはClaude Code上でしか使われていない。
今の時点で汎用Hooks抽象化を作るのは、tree-sitterのときと同じ「実例が無いのに
一般化する」誤りになる。今はClaude Code Hooksの設定ファイル＋スクリプトを
そのまま書けばよい。

### どのイベントを対象にするか

- 候補2・4（drift検知）: **PostToolUse**で確定
- 候補5・6（実行前拒否）: **PreToolUse**で確定（ブロックという性質上、
  他の選択肢が無い）
- セッション終了時（Stop）・定期実行は、**Go候補4件のどれにも不要**。狙って
  いるのは「直近の1回の書き込みが直近の1つの規律に違反していないか」という
  即時フィードバックであり、まとめて後で確認する意味がない

### 追記: 候補8（新規）— セッション終了時の一括drift確認

Stop hookで全driftチェック（5〜6種）を一括実行する案は、今回のGo候補4件とは
別の価値（セッション全体の総括的な健全性確認）を持つ可能性がある。既存の
Go候補が「直近の1件」を即座に見るのに対し、候補8は「セッション全体を通して
見落としが無いか」を見る、粒度の異なる検知として8番目の候補に追加する。
設計は未着手。
...
Go候補5・4・2・6を`.waffle/hooks/`配下に実装し、`.claude/hooks/`へシンボリック
リンク（既存の4 advisor Skillと同じ配布パターン）、`.claude/settings.json`
（新規・git管理対象、`.claude/settings.local.json`は個人設定でgitignore対象
のため使わない）に実際に登録した。

- `protect-document-json.py`（候補5、PreToolUse）: document.json直接編集を拒否
- `enforce-spec-first.py`（候補6、PreToolUse）: spec無し新規schema作成を拒否
  （spec.status=VALIDATED以上を「合意済み」の代替指標に使う）
- `check-drift-on-write.py`（候補2・4合体、PostToolUse）: 書き込まれたファイル
  パスに応じて`check-usecase-class-drift`/`check-operation-drift`/
  `check-aggregate-class-drift`/`check-domain-service-drift`/
  `check-scenario-drift`を自動発火し、検出があるときだけ結果を返す（クリーン
  なら沈黙）

実際のペイロードで4パターンとも動作確認済み（Entity書き込みで既知の
`agg-document`未実装を正しく検出、業務サービス書き込みでクリーン時の沈黙を
確認等）。**既知の限界**: `check-scenario-drift`が1回の呼び出しで1つのtest
ファイルしか比較できないため、acceptance/integrationの片方だけに書き込むと、
もう片方のシナリオが「missing」と誤検知されるノイズが出る。usecase側
...
## 候補5拡張: document.json/schema.jsonの「読み」もPreToolUseで防ぐ（2026-07-13）

候補5（`protect-document-json.py`）はEdit/Writeでの直接「書き」しか見ておらず、
CodingSchema拡張作業中に実際にAI（Claude）がpython3(`json.load`)・cat・grep・
headでdocument.json・schema.json（`src/waffle/domain/model/**/*.json`）を直接
「読んで」いたことをユーザーが実行ログから指摘して発覚した。CLAUDE.mdの運用ルール
表には元々「直接読み書きしない」と書かれていたが、テキストのルールを知っていても
実行時に読み飛ばして破ってしまうため、文章の増し書きでは再発防止にならないと
ユーザー・AI双方で合意した。Waffle自身のHarness原則（AIは値だけ、構造は機械が
守る）に沿って、候補5と同じ機構をBash（cat/grep/python3等での直接読み取り）と
Read tool、対象パス（document.jsonに加えschemaファイルも）の両方に拡張する
方針にした。

**実装**: `protect-raw-json-access.py`を新設（`.waffle/hooks/`→`.claude/hooks/`
へ既存と同じシンボリックリンクパターンで配置）。

- 対象パス: `.waffle/documents/**/*.json`（document.json）と
  `src/waffle/domain/model/**/*.json`（schema）の両方
- `Read` tool: `file_path`が対象パスに一致すれば無条件deny
- `Bash` tool: コマンド文字列に対象パスへの参照＋生読み取り系コマンド
  （cat/head/tail/less/more/bat/python(3)/node/jq/sed/awk/grep/perl/ruby/php/
  tac/nl/strings/xxd/od のいずれか）が両方含まれていればdeny。ただし
  `git`で始まるコマンド（diff/show/log等の履歴確認）と、
  `waffle query/render/validate/scaffold/patch-schema/check-*/
  scan-source-code/lint-docstring`というCLIサブコマンド呼び出しの形は
  正規の経路として除外する
- 落とし穴: 当初`waffle`という単語の有無だけで正規/非正規を判定していたが、
  対象パス自体に`.waffle/`という文字列が含まれるため、`cat .waffle/
  documents/....json`のようなコマンドが「wafffleを含む」という理由で誤って
  許可されてしまうバグがあった。CLIサブコマンド名まで含めた正規表現に
  修正して解消（`waffle\s+(query|render|validate|scaffold|patch-schema|
  check-[a-z-]+|scan-source-code|lint-docstring)`）
- 14ケース（deny 6件・allow 8件）を手動で構築したペイロードで検証し、
  全て期待どおりの判定になることを確認してから本番の`.claude/settings.json`
  （PreToolUseに`Read|Bash`マッチャーを追加）に登録した

---

## 候補6の撤回・書き直し: PreToolUse denyからPostToolUse通知へ（2026-07-14）

HandoffSchema導入の検討中、`enforce-spec-first.py`（候補6、`HandoffSchema/v1.json`の
新規Writeを、それを参照するVALIDATED以上のspecが無いという理由でdeny）が実際に発火し、
これが誤検知であることが議論の末に判明した。ddd-advisor/tech-lead-advisorへの相談の結果、
HandoffSchemaは「新設usecase不要」（既存のsd-document-management/sd-schema-managementの
汎用CRUDに吸収される）という設計が正しいと判定されたため、そもそも「HandoffSchemaを
参照するusecase spec」という、候補6が要求する証跡自体が存在しえないケースだった。

### 検討した代替案とその棄却理由（結論だけでなく全て残す）

1. **証跡の対象をbrainstormドキュメントにも広げる案 → 棄却**
   ブレストは定義上「発散」の場（今回のHandoffSchemaのブレスト自体、複数の破棄された
   アイデアを経由して収束した）であり、名前が言及されていることは「合意した」証拠に
   ならない。むしろ未決着の途中経過も同様に言及されるため、区別がつかない
2. **Hookの対象をDomainSpecSchema（業務モデリング系）に絞り、Waffle自身のインフラ・
   運用系schema（Coding/Agent/Skill/Template/Knowledge/Handoff等）を対象外にする案
   → 一度提案したが、schemaファイルは全て`src/waffle/domain/model/`配下の同形式で
   あり、ファイルパスだけからこの2分類を機械的に判定する手段が無いため頓挫**
3. **HandoffSchema自体にDRAFT/RESOLVEDのようなstatus語彙を持たせ、RESOLVEDなHandoff
   ドキュメントを証跡として使う案 → 棄却**
   結局そのstatusフィールドもAI（実行主体）自身が書き込む値であり、「合意した」ことの
   証明にはならない。ブレスト参照案と同じ欠陥（自己申告）を形を変えて再生産しただけ
4. **新規schemaファイルへの書き込みを常に1回目はdenyし、2回目以降を許可する
   「二段階確認」案 → 棄却**
   1回目のdenyを見た後、ユーザーに実際に確認するかどうかは結局AIの自己申告に委ねられる
   （同じターン内で無言のまま2回連続でツールを呼べば形式上は突破できる）。「無言の
   一発実装」を「無言の二発実装」に変えるだけで、防ぎたい失敗モードを実質防げない上、
   「Hookがあるから大丈夫」という偽の安心感を生む点でむしろ悪い

### 到達した結論

**「tool_inputの中身だけから『人間が本当に合意したか』という社会的事実を検証すること
は、この環境（AIが全てのツール実行の主体である）では原理的に不可能。** どんな形式の
証跡ファイルを要求しても、それはAI自身が書けてしまう自己申告に帰着する。候補5・候補6
拡張（`protect-raw-json-access.py`）が機能しているのは、判定対象が「tool_inputの外形
（ツール名・パスパターン）」という、例外の無い絶対規則だからであり、候補6が判定しよう
としている「合意・意図」は本質的に別の種類の問題だった。

一方で、Waffleは基本spec-first+TDDで実装しており、Gherkinシナリオ（AcceptanceScenarios
等）はspecが正本であるため、specが無ければAIはそもそも何を根拠にテストを書けばよいか
分からず、自然にspecの有無を確認する動きになる（＝一次防御は既にプロセス自体に内蔵
されている）、という指摘もユーザーからあった。ブロッキングは「それが無いと必ず困る」
場面に限るべきで、それ以外は通知で足りる（通知を見ればAIは自身の振る舞いを見直せる）
という運用方針にも合意した。

### 実装した内容

`enforce-spec-first.py`をPreToolUse（deny）からPostToolUse（`additionalContext`による
通知のみ、ブロックしない）へ書き直した。判定ロジック自体（VALIDATED以上のspecが
schemaRefを参照しているか）はほぼそのまま流用し、`.claude/settings.json`の登録先を
`PreToolUse`から`PostToolUse`（`check-drift-on-write.py`と同じ`Edit|Write`マッチャー）
へ移動した。これにより「specがあるのに引っかかる」という誤検知の実害（ブロックによる
手戻り）は無くなり、「本当にspecが無いまま新規schemaを書いた」ケースはレビュー時に
拾える記録として残る。
