...
TREE = ('<svg viewBox="0 0 780 420" role="img" aria-label="フォルダは何を入れるかを名乗り、実体の名前はその中に置く">'
 '<text x="20" y="20" font-size="10.5" font-weight="700" fill="var(--key)">B　業務でフォルダを切る</text>'
 + _box(20, 30, 420, 336, "specs/", M, 1.2)
 ...
 + _box(32, 320, 200, 34, "integrations/", A, 1.2, 8, 14, 8.5, True, None)
 + _f(44, 348, "予約-請求.json　文脈どうしの連係", M, 8)
 + _f(244, 336, "areas.json", A, 9)
 + _f(244, 350, "業務領域 ── 文脈と1対1とは限らない", M, 8)
 + '<text x="460" y="20" font-size="10.5" font-weight="700" fill="var(--muted)">A　層でフォルダを切る</text>'
 + _box(460, 30, 300, 336, "specs/", M, 1.2)
 + "".join(
     _box(472, 50 + i * 44, 276, 36, lbl, D if "予約" in lbl else M, 1.1, 8, 14, 8.5)
     + _f(484, 50 + i * 44 + 29, sub, M, 8)
 ...
 + _f(472, 336, "同じ業務の名前が、6つのうち5つに現れる", D, 8.5)
 + _f(472, 352, "── 文書が兄弟に散る", D, 8.5)
 + '<line x1="20" y1="378" x2="760" y2="378" stroke="var(--rule)"/>'
 + _f(20, 396, "フォルダは「何を入れるか」を名乗り、実体の名前はその中に置く ── contexts/ の中に 予約/ が入る", M, 9)
 + '<text x="20" y="414" font-size="9" fill="var(--key)">人が読む単位は業務である（論点2 ・ 論点15）── '
 ...
 '<b>業務領域は、この入れ子には入らない</b> ── 文脈との対応が1対1とは限らないので、対応を書いた1本として置く。')
