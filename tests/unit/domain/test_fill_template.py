...
def test_配列の要素がプリミティブの配列プロパティを持つときもプレースホルダー配列にする():
    ...
    When build_fill_templateで走査しoverlay_placeholdersで合成する
    Then そのプロパティも1件分のプレースホルダー文字列を含む配列になる（文字列1個に潰れて
         「'bullets' は配列であるべきですが str でした」というMalformedContentErrorを
         レンダラ側で起こさない）
    """
    content = {
        "type": "object",
        ...
        "properties": {
            "steps": {
                ...
                "items": {
                    "type": "object",
                    "properties": {
                        "title": {"type": "string", "x-prompt-write": "手順名"},
                        "bullets": {
                            "type": "array",
                            "x-prompt-write": "補足の箇条書き",
                            "items": {"type": "string"},
                        },
                    },
                },
...
def test_discriminatorごとの入れ子x_prompt_writeはspec_kindに対応する文言だけを使う():
    ...
    Then そのspec_kindに対応する文言だけがpromptとして使われる（他kind分の文言は混入しない）
    """
    content = {
        "type": "object",
        "required": ["title"],
        "properties": {
            "title": {
                "type": "string",
                "x-prompt-write": {"usecase": "usecase向けの文言", "aggregate": "aggregate向けの文言"},
            },
        },
    }

    entries = build_fill_template({}, content, spec_kind="usecase")
...
def test_discriminatorの入れ子x_prompt_writeでspec_kindが指定されないときは空文字にする():
    ...
    Then プロンプトが辞書のまま漏れ出さず、空文字になる（discriminatorを持たないschemaで
         誤って辞書がそのままプレースホルダーに埋め込まれる事故を防ぐ）
    """
    content = {
        "type": "object",
        "required": ["title"],
        "properties": {
            "title": {"type": "string", "x-prompt-write": {"usecase": "usecase向けの文言"}},
        },
    }
...
def test_x_prompt_writeがフラットな文字列のときはspec_kindを渡しても従来どおり使う():
    ...
    Then そのフィールドの文字列がそのままpromptとして使われる（discriminator分岐が無いフィールドは
         spec_kindの影響を受けない）
    """
    content = {
        "type": "object",
        "required": ["title"],
        "properties": {"title": {"type": "string", "x-prompt-write": "共通の文言"}},
    }
...
def test_配列要素内のdiscriminatorごとの入れ子x_prompt_writeもspec_kindで解決する():
    ...
    When build_fill_templateにspec_kindを渡して走査する
    Then element内のプロンプトもそのspec_kindに対応する文言になる
    """
    schema = {
        "$defs": {
            "ErrorItem": {
                ...
                "properties": {
                    "code": {
                        "type": "string",
                        "x-prompt-write": {"usecase": "usecaseのエラーコード", "aggregate": "aggregateのエラーコード"},
                    },
                },
            },
        },
    ...
    entries = build_fill_template(schema, content, spec_kind="aggregate")
