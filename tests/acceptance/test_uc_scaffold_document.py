...
_MIGRATE_FIXTURE_DIR = Path("src/waffle/domain/model/TestMigrateSchemaFixture")
_MIGRATE_FIXTURE_V1 = _MIGRATE_FIXTURE_DIR / "v1.json"
_MIGRATE_FIXTURE_V2 = _MIGRATE_FIXTURE_DIR / "v2.json"
_MIGRATE_DOC_PATH = ".waffle/documents/test-migrate-doc.json"
...
def _migrate_fixture_schema() -> dict:
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "type": "object",
        "required": ["documentId", "documentType", "schemaRef", "content"],
        "properties": {
            "documentId": {"type": "string"},
            "documentType": {"const": "TestMigrate"},
            ...
            "content": {
                ...
                "properties": {
                    "title": {
                        ...
                        "properties": {"blockType": {"const": "Title"}, "title": {"type": "string"}},
                    }
                },
            },
        },
...
def _write_migrate_fixtures() -> None:
    _MIGRATE_FIXTURE_DIR.mkdir(parents=True, exist_ok=True)
    schema_text = json.dumps(_migrate_fixture_schema(), indent=2, ensure_ascii=False) + "\n"
    _MIGRATE_FIXTURE_V1.write_text(schema_text, encoding="utf-8")
    _MIGRATE_FIXTURE_V2.write_text(schema_text, encoding="utf-8")


def _write_migrate_document(schema_ref: str = "TestMigrateSchemaFixture/v1") -> None:
    FsDocumentRepository().save(_MIGRATE_DOC_PATH, {
        "documentId": "test-migrate-doc",
        "documentType": "TestMigrate",
        "schemaRef": schema_ref,
        "content": {"title": {"blockType": "Title", "title": "テスト用文書"}},
    })
...
def test_migrate_schemaはschemaRefを新版へ書き換える():
    ...
    _write_migrate_document()

    result = _engine().run("migrate_schema", {
        "documentPath": _MIGRATE_DOC_PATH,
        "schemaRef": "TestMigrateSchemaFixture/v2",
    })
    assert isinstance(result, Ok), result
    assert result.value["changed"] is True

    reloaded = FsDocumentRepository().load(_MIGRATE_DOC_PATH)
...
def test_migrate_schemaは同じ版への書き換えに対して冪等である():
    ...
    """
    _write_migrate_fixtures()
    _write_migrate_document(schema_ref="TestMigrateSchemaFixture/v2")
...
def test_migrate_schemaは解決できないschemaRefをINVALID_SCHEMA_REFとして拒否する():
    ...
    _write_migrate_fixtures()
    _write_migrate_document()
    before = FsDocumentRepository().load(_MIGRATE_DOC_PATH)

    result = _engine().run("migrate_schema", {
        "documentPath": _MIGRATE_DOC_PATH,
        "schemaRef": "NoSuchSchema/v1",
    })
    assert isinstance(result, Err), result
