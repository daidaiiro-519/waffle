...
def test_識別子を複数箇所にわたってリネームする():
    ...
    Then $defsキー名・blockType const・プロパティキー名・required配列内エントリ・$ref参照文字列がすべて新短縮名に一貫してリネームされる
    """
    result = _engine().run("rename_block", {"schemaRef": _SCHEMA_REF, "oldName": "Title", "newName": "Heading"})
    assert isinstance(result, Ok), result
    written = json.loads(_FIXTURE_PATH.read_text(encoding="utf-8"))
    assert "HeadingBlock" in written["$defs"]
    assert "TitleBlock" not in written["$defs"]
    assert written["$defs"]["HeadingBlock"]["properties"]["blockType"]["const"] == "Heading"
    assert "heading" in written["$defs"]["SomeContent"]["properties"]
    assert "heading" in written["$defs"]["SomeContent"]["required"]
...
def test_requiredへの追加はBACKWARD_INCOMPATIBLEとして拒否される():
    """
