...
**確かめ方は1つ。その段落を、表か図に置き直せるかを試す。**
**置き直せるなら、散文で書くべきではない。**

| 段落に在るもの | 置き直せる形 |
|---|---|
| **対比**（一方は〜、他方は〜） | 表。列は比べる軸 |
| **条件と結果**（〜なら〜する） | 表。左に条件、右に結果 |
| **手順**（まず〜、次に〜） | 番号つきの並び。分岐があれば図 |
| **分岐・流れ・包含** | 図（mermaid か SVG） |
| **理由**（〜だから〜である） | **散文のままでよい。**置き直すと連鎖が切れる |

**箇条書きは、本当に並列なときだけ使う。**

| 並べたものに在るもの | 箇条書きでよいか |
|---|---|
| 順序がある | **よくない。**番号つきにするか、地の文で繋ぐ |
| 因果がある | **よくない。**散文で連鎖として書く |
| 項目ごとに、同じ軸の値が並ぶ | **よくない。**表にする。軸が列になる |
| どれも独立していて、順序も因果も無い | よい |

### 直し方

| 出方 | 直す向き |
|---|---|
| 対比が散文にたたまれている | **表にする。**「一方で」「また」で繋いだ文は、たいてい表になる |
| 条件が散文に並んでいる | **表にする。**3つ以上なら、読み手は引くだけで済む |
| 箇条書きに順序や因果がある | **番号つきにするか、散文へ戻す** |
| 図を文字で描いている | **mermaid か SVG にする。**桁を空白で揃えるのは、表でも図でもない |
| コードブロックに、コードでも出力でも木構造でもないものが在る | **表か本文へ出す** |

### 失敗の例

| 悪い | なぜ悪いか | 良い |
|---|---|---|
| 旧は行の先頭で除外していた。一方、新は種別ごとに宣言する。また、旧は重複が出たが、新は出ない。 | **対比が散文にたたまれている。**比べる軸を、読み手が組み立て直すことになる | 列を「旧」「新」、行を「除外の仕方」「重複」にした表 |
| 桁を空白で揃えて並べたもの | **表に見えるが表ではない。**幅が変われば崩れ、読み上げも効かない | マークダウンの表 |
| 「まず原文を落とす」「次に識別子を書き出す」「最後に照合する」を、印つきの箇条書きで並べる | **順序があるのに、並列の形で置いている** | 番号つきの並び |

### 機械が見るもの

**この検査が見られるのは、文字で図を描いていないかだけである。**

| 検査 | 見るもの |
|---|---|
| ASCII で図を描いている | 箱の角と、横罫の連なり。**二倍ダッシュと木構造は図ではない** |

**その情報が構造を持つかどうかは、機械には見えない。**読んで決める。
...
```mermaid
flowchart TD
    a["論<br/>何を言うかを決め、重複と散らばりを消す"]
    s["構造<br/>情報の型に合う形へ置き直す"]
    b["語彙<br/>形が決まると、語の揺れが見える"]
    c["文調<br/>最後に1文ずつ直す"]
    g{"3つのゲート"}
    a --> s --> b --> c --> g
    g -- "未処理か新規が残る" --> a
...
**構造は論の次に置く。**何を言うかが決まる前に形を選ぶと、決まったあとで組み直すことになる。
