...
ここはWaffle固有の語彙（Document/Schema/asserts等）を一切知らない。
知っているのは「節点(nodes)・辺(edges)・囲み(groups)」という、どんな
グラフ図にも共通する一般名詞だけ。Waffle側の語彙からこの形へ変換する
役目は、この核の外（呼び出し側のアダプタ）が担う。
