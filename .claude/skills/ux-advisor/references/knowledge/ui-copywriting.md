...
文言は常に利用者の視点から書く。操作を表す言葉は、システム内部の仕組み（アーキテクチャ・処理名）ではなく、利用者が認識し操作対象として扱う言葉を使う。能動態を基本とし、婉曲・受動的な言い回しを避ける。同じ操作を指す言葉は画面をまたいで一貫させる——例えば操作ボタンが「公開する」なら、完了後の確認も「公開しました」のように同じ語幹を使う。文体は会話的で平易、気の利いた言い回しより具体的で分かりやすい表現を優先し、ブランド・想定利用者に応じてトーンを調整する。
