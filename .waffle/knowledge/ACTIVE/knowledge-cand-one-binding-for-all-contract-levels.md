...
- 受け入れ条件は「主張」で切り、振る舞いのシナリオは「流れ」で切る。同じものを違う軸で切った2つの分割の間の対応は、一般に多対多になる。
...
- 契約の対は、それを担う要素が持つ。文書そのものが担い手なら条件も筋書きも文書レベルに来る（業務ユースケース・集約）。文書の中に担い手が複数いるなら、条件も筋書きも要素の中に持つ（業務サービス）。
...
未検証の留保が3点ある。
(1) 対の欠けが実測で3箇所ある。集約は受け入れ条件（不変条件）17件に対しシナリオ12件で、5件分が足りない。業務サービスはシナリオ5件があるのに受け入れ条件そのものが無い（責務の散文だけ）。そして対応の宣言は全体では594件中526件（88%）が記入されているものの、書かれている値が鍵として使えない——集約では不変条件の原文の写し、業務ユースケースでは条件の原文と一字も一致しない短縮ラベルになっている。このためどの条件にシナリオが無いのかを機械的に特定できない。この3つが埋まった状態は、まだ一度も実現していない。
