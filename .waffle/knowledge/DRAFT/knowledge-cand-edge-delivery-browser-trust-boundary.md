...
「端で判定する」構成の妥当性は、閲覧のたびに裏側を起こしたくない・読む人に利用者登録を求めない、という2つの前提に依存する。どちらかが外れれば裏側での判定が素直になる。また、端の実行環境ごとの上限値（コードの大きさ・実行時間・保管への到達手段）は提供者と時期によって変わるため、数値そのものは持ち出さず、制約の存在だけを持ち出すこと。
