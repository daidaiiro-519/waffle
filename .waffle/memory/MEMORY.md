...
- [抽象は情報の削減ではない](feedback_abstraction_is_not_reduction.md) — 要素は残し、共通概念の区画で束ねる。図なら囲む
- [フックは例外なく仕様で管理する](feedback_hooks_are_spec_managed_without_exception.md) — 判断か配線かで免除しない。自前型は抜け穴として働いていた
- [schemaの指示でknowledgeを広げない](feedback_schema_prompts_must_not_widen_knowledge.md) — 免除の文言を書く瞬間が合図。HookSchemaとDomainSpecSchemaで同型の抜け穴を実測
