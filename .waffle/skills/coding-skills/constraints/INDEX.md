...
| `arch.data-port` | architecture＝data-port | `test-boundaries.md` | 確かめる単位の境界 | 4 |
...
| `lang.rust+arch.data-port` | language＝rust ／ architecture＝data-port | `test-placement.md` | テストの置き場所 | 4 |
...
| `purpose.backend-api` | purpose＝backend-api | `stack.md` | 採用するアーキテクチャ・実行環境・実行時の依存 | 1 |
| `purpose.backend-api` | purpose＝backend-api ／ runtime＝resident | `test-strategy.md` | テストの重心・計画・観点の採否 | ─ |
| `purpose.hook` | purpose＝hook | `acceptance.md` | 保証する振る舞い | 5 |
...
| `purpose.mcp-server` | purpose＝mcp-server ／ runtime＝resident | `test-strategy.md` | テストの重心・計画・観点の採否 | ─ |
