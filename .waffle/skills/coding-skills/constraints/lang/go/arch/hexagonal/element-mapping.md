---
id: lang.go+arch.hexagonal.element-mapping
...
**構成要素をパッケージで表し、依存の向きは import で守る。抽象は使う側のパッケージが定める。**
...
| `internal/app` パッケージ | 業務操作と二次のポートの宣言 | `internal/model` | `internal/adapter` |
| `internal/adapter/inbound` | 入力アダプター | `internal/app`、外部モジュール | `internal/model` の内部 |
| `internal/adapter/outbound` | 二次のポートの実装 | `internal/app`、外部モジュール | `internal/app` の内部 |
| `cmd/<名前>` | 結線と起動 | すべて | ─ |
...
| 業務操作 | `internal/app` パッケージ | 二次のポートを **自分の側で** interface として定める |
| 二次のポート | `internal/app` の中の interface | 実装側には置かない |
| 入力アダプター | `internal/adapter/inbound/<技術>` | 外部技術を import してよい |
| 出力アダプター | `internal/adapter/outbound/<技術>` | interface を満たす具体型を置く |
...
flowchart TB
    subgraph internal[internal]
        direction TB
        model[package model]
        app["package app<br/>二次のポートの interface を持つ"]
...
| 境界 | 渡す形 | 変換する場所 |
|---|---|---|
| 外部 → 入力アダプター | 外部の形式（JSON など） | 入力アダプター |
| 入力アダプター → 業務操作 | `app` が定める入力の構造体 | 入力アダプター |
| 業務操作 → 二次のポート | `model` の型 | 変換しない |
