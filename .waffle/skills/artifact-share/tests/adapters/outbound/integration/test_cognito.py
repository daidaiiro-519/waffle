...
def test_署名の無いものは通さない():
    ...
    ただしこの筋書きは、署名が空であることでも落ちる。署名方式の検査そのものが
    効いているかは、次の検証が分けて確かめる。
    """
    tok = token(claims(), header={"alg": "none"}, signature=b"")
    assert verify(tok) is None


def test_署名が通っても方式が違えば拒む():
    """署名方式を証明の側に選ばせない。

    署名の検証を必ず通る状態にしてから、alg だけを変える。こうすると拒む理由が
    方式の検査ひとつに絞られる。前の検証は署名が空でも落ちるため、方式の検査を
    外しても気づけなかった（変異を注入して実際に確かめた）。
    """
    tok = token(claims(), header={"alg": "HS256"})

    assert verify(tok) is None      # 既定の検証器は署名を必ず通す
