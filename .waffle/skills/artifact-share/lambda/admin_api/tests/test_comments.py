...
def test_寄せられた順に読める():
    ...
    got = comments.read(deps.artifacts, deps.comments, ME, aid)
...
def test_保存されている形のまま返る():
    ...
    c = comments.read(deps.artifacts, deps.comments, ME, aid)["comments"][0]
...
def test_他人のものは読めない():
    ...
    with pytest.raises(manage.ManageError) as x:
        comments.read(deps.artifacts, deps.comments, SOMEONE_ELSE, aid)
...
def test_管理者は他人のものも読める():
    deps, aid = setup()
    post(deps, aid, 1700000001, "田中", "本文")
    assert len(comments.read(deps.artifacts, deps.comments, ADMIN, aid)["comments"]) == 1
...
def test_読んでも何も変わらない():
    ...
    comments.read(deps.artifacts, deps.comments, ME, aid)
    comments.read(deps.artifacts, deps.comments, ME, aid)
