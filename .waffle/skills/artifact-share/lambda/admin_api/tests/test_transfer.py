...
def test_管理者は他人のものも一覧できる():
    deps, r = setup()
    ids = [row["artifactId"] for row in manage.list_artifacts(deps.store, ADMIN)["artifacts"]]
...
def test_一覧には誰が公開したかが分かる():
    ...
    deps, r = setup()
    assert manage.list_artifacts(deps.store, ADMIN)["artifacts"][0]["uploadedBy"] == X.id
...
def test_管理者は他人のものを公開停止_再開_再発行できる():
    ...
    manage.suspend(deps.store, deps.keys, deps.now, ADMIN, aid)
    assert meta_of(deps, aid)["status"] == "disabled"

    manage.resume(deps.store, deps.keys, deps.now, deps.viewer_domain, ADMIN, aid)
    assert meta_of(deps, aid)["status"] == "active"

    again = manage.reissue_token(deps.store, deps.keys, deps.now, deps.viewer_domain, ADMIN, aid)
    assert again["token"]
...
def test_管理者でも他人の中身は差し替えられない():
    ...
    with pytest.raises(manage.ManageError) as x:
        manage.replace_content(deps.store, deps.now, deps.viewer_domain, ADMIN, r["artifactId"], HTML.replace("本文", "別"))
...
def test_第三者には見つからないものとして拒む():
    ...
    deps, r = setup()
    with pytest.raises(manage.ManageError) as x:
        manage.suspend(deps.store, deps.keys, deps.now, Y, r["artifactId"])
...
def test_移した先が手入れできるようになる():
    deps, r = setup()
    result = manage.transfer(deps.store, deps.directory, deps.now, ADMIN, r["artifactId"], Y.id)

    assert result["event"] == "ArtifactTransferred"
    assert meta_of(deps, r["artifactId"])["uploadedBy"] == Y.id
    manage.suspend(deps.store, deps.keys, deps.now, Y, r["artifactId"])          # Yが扱える


def test_移す前の人は扱えなくなる():
    """引き継ぎは移動であって複製ではない"""
    deps, r = setup()
    manage.transfer(deps.store, deps.directory, deps.now, ADMIN, r["artifactId"], Y.id)

    with pytest.raises(manage.ManageError) as x:
        manage.suspend(deps.store, deps.keys, deps.now, X, r["artifactId"])
    assert x.value.code == "ARTIFACT_NOT_FOUND"
...
def test_管理者でない者は移せない():
    deps, r = setup()
    with pytest.raises(manage.ManageError) as x:
        manage.transfer(deps.store, deps.directory, deps.now, X, r["artifactId"], Y.id)
...
def test_招かれていない人へは移せない():
    ...
    deps, r = setup()
    with pytest.raises(manage.ManageError) as x:
        manage.transfer(deps.store, deps.directory, deps.now, ADMIN, r["artifactId"], "no-such-person")
