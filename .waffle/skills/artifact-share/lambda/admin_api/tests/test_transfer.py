...
def setup():
    ...
    result = publish.publish(
        ...
        publish.Deps(store=store, keys=keys, identify=lambda _t: X.id,
...
def test_管理者は他人のものを公開停止_再開_再発行できる():
    ...
    manage.suspend(deps, ADMIN, aid)
    assert meta_of(deps, aid)["status"] == "disabled"

    manage.resume(deps, ADMIN, aid)
    assert meta_of(deps, aid)["status"] == "active"

    again = manage.reissue_token(deps, ADMIN, aid)
    assert again["token"]
...
def test_管理者でも他人の中身は差し替えられない():
    ...
    with pytest.raises(manage.ManageError) as x:
        manage.replace_content(deps, ADMIN, r["artifactId"], HTML.replace("本文", "別"))
...
def test_第三者には見つからないものとして拒む():
    ...
    deps, r = setup()
    with pytest.raises(manage.ManageError) as x:
        manage.suspend(deps, Y, r["artifactId"])
...
def test_移した先が手入れできるようになる():
    deps, r = setup()
    result = manage.transfer(deps, ADMIN, r["artifactId"], Y.id)

    assert result["event"] == "ArtifactTransferred"
    assert meta_of(deps, r["artifactId"])["uploadedBy"] == Y.id
    manage.suspend(deps, Y, r["artifactId"])          # Yが扱える


def test_移す前の人は扱えなくなる():
    """引き継ぎは移動であって複製ではない"""
    deps, r = setup()
    manage.transfer(deps, ADMIN, r["artifactId"], Y.id)

    with pytest.raises(manage.ManageError) as x:
        manage.suspend(deps, X, r["artifactId"])
    assert x.value.code == "ARTIFACT_NOT_FOUND"
...
def test_管理者でない者は移せない():
    deps, r = setup()
    with pytest.raises(manage.ManageError) as x:
        manage.transfer(deps, X, r["artifactId"], Y.id)
...
def test_招かれていない人へは移せない():
    ...
    deps, r = setup()
    with pytest.raises(manage.ManageError) as x:
        manage.transfer(deps, ADMIN, r["artifactId"], "no-such-person")
