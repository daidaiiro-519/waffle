...
# UIモック生成とDesign.md作成を自前AWS環境で共有する対応を行うSkill：design-share
...
ヒアリングで得た発話を、references/knowledge/nl-feature-routing.md の5カテゴリ（視覚・印象／情報構造・タスク型／利用文脈・習熟度／トーン・ボイス／リスク・不可逆性）に照らし、「どの部分が・どのカテゴリで・どこへ送られるか（8語のどれ、またはprogressive-disclosure/ui-copywriting等の既存knowledge）」を一文で言い切ってからStep2以降へ進む。硬い対応表として機械照合せず、発話ごとに言葉で言い切る（8語側の『理由を言い切る』と対になる、注入側のゲート）。
...
- 【UIモックの標準サイクル】最良のUIモックは〈生成→敵対的測定→修正→学びの還元〉サイクルで作り、これは design-share の中で完結する。自己批評（理由を言い切る／DESIGN.mdの掟に照らす）だけで完成としない。生成したモックに、このスキルが同梱するUX専門知識（references/knowledge/ 特に frontend-design-principles＝ux-advisor由来）を土台にした独立した敵対的測定をかける: 明示bar（商用級か／核の価値／ありきたりでないか）に照らし、実物と数値evidence（コントラスト計算等）で欠陥を具体箇所で名指しし、verdict＋順位付きmust/should-fixを得る→修正→必要なら再測定で閉じる→学びを知識/アセットへ還元。生成者は自己採点できないため独立性が必須だが、独立性は“別Skillを呼ぶこと”でなく“別pass・敵対フレーミング（忖度なし/出荷基準/refute既定）・実物の読解”で担保する（生成時の文脈と自己弁護を持ち込まない別レビューとして回す。必要なら同梱UX知識を読ませたサブエージェントを敵対レビュアとして使う）。※外部のux-advisor Skillを呼ぶのではなく同梱UX知識による自前測定なので、自己完結（実行時に他Skillを呼ばない）と両立する。 【敵対的測定は3層の直列ゲートで行う】1回の測定で商用出荷可否とありきたり回避と細部の破綻を同時に問うと、『条件付きYes』の中身が曖昧になり、下の層の欠陥（例: lang属性欠落）が上の層の良さに埋もれて見落とされる。3層を順に測定し、上位層がFAILしたら下位層は測定しない（差し戻して再生成する）: ①コンセプト層＝判断の出どころが題材にあるか・ありきたりでないか（題材に根ざす／ありきたりを避ける／主役を立てる／理由を言い切る／round-trip）。ここがFAILなら見た目をどれだけ磨いても無駄なので即差し戻す。②レイアウト層＝サーフェス別の情報設計が目的に噛み合っているか（SaaS=要約→明細・状態の形符号化、LP=ヒーローの訴求とCV導線、コーポレート=IA・信頼設計の情報粒度・固有名詞/資格の細部精度）。③デザイン層＝クラフトと実装が商用の仕上げに達しているか（書体/アイコン/チャートの技術精度、コントラスト・フォーカス等のa11y下限、lang/full-doc等の文書構造）。全層PASSして初めて『商用レベル到達（Yes）』と判定する。fix後は必ずその層から再測定し、『直したはず』を生成者の自己申告のまま確定にしない。 ③デザイン層はさらに2つのサブゲートに分ける。③a視覚クラフト（最優先で見る）＝はみ出し・見切れ・余白の欠落や衝突（overflow、要素の重なり、paddingゼロで文字が枠に接触、marginの相殺で密着しすぎ/離れすぎ）といった視覚的に目立つ物理的な崩れを最初に確認する。次いでSVGの歪み（preserveAspectRatio異方性・真円が卵形になる等）、コントラスト・フォーカス可視・状態の三重符号化などa11y下限、lang/full-doc等の文書構造。③bCSS技術品質＝セレクタ詳細度の衝突（.sectionのような種類セレクタと.ctaのような要素セレクタがpadding/marginを打ち消し合っていないか）、レスポンシブは複数ビューポート（狭幅・中間幅・広幅）で実際に崩れを確認する（静的コード読解だけで済ませない）、保守性（命名一貫性・重複ルール・デッドCSS・!important濫用）。 生成後、共通コンテナクラス（.wrap等）と組み合わせる全要素をgrepで洗い出し、同じ要素にpaddingを重複定義していないか（詳細度や定義順で共通クラスの横方向paddingを丸ごと上書きしていないか）をメディアクエリ内の再定義も含めて機械的に確認する。モバイルナビは既定でハンバーガー＋ドロワー等の発見可能な標準パターンを使い、横スクロール行のような独自パターンは理由を言い切れる場合のみ採用する。
- コーポレート/信頼設計中心のモックでは、敵対的測定の物差しにビジュアルクラフトだけでなく信頼設計軸（固有名詞・資格表記・登録番号等の細部精度、写真依存プレースホルダーの全箇所一貫性、実在の法人・個人への信頼を毀損する誤植）を明示的に含める。ビジュアルが整っていても細部の誤植・非一貫性が信頼を一箇所で崩すため、生成後に固有名詞/資格/数値だけを独立して読み返すパスを設ける。
- 【自然言語の写像ゲート】ユーザー発話をDesign.md確定・トークン決定に反映する前に、references/knowledge/nl-feature-routing.md の5特徴量カテゴリ（視覚・印象／情報構造・タスク型／利用文脈・習熟度／トーン・ボイス／リスク・不可逆性）に照らし、「この発話のどの部分が・どのカテゴリで・どこへ送られるか（8語のどれ、またはprogressive-disclosure/ui-copywriting等の既存knowledge）」を一文で言い切ってから展開する。硬い対応表として機械的に照合せず、発話ごとに言葉で言い切る（理由を言い切るの対になる注入側のゲート）。確定したトークン・構成要素には、それがどの発話・どの特徴量カテゴリに由来するかを一言添える（DESIGN.mdのコメント等）。これにより注入の質を事後検証でき、展開側の成果物のトレーサビリティも同時に補う。
...
- `references/knowledge/nl-feature-routing.md`: 自然言語(ユーザー発話)を8語の概念層または既存knowledgeへ写像する変換層。8語自体は語彙(受け皿)であって変換ロジックではないため、この変換層が写像を担う。5特徴量カテゴリ(視覚・印象/情報構造・タスク型/利用文脈・習熟度/トーン・ボイス/リスク・不可逆性)と、それぞれの送り先を定義する
