...
### Step 4: パターン単位でデプロイしURL＋トークンを発行する
...
### Step 5: コメントを回収しDesign.mdへ反映する

scripts/ds.sh export <slug> のzip、または aws s3 sync でコメントJSONを取得して要点を整理し、Design.mdへの差分として反映する。必要ならパターンを再生成・再デプロイして改善サイクルを回す。

### Step 6: 公開中パターンを管理する

管理操作はCLI（scripts/ds.sh）・Web UI（ds.sh console、localhostのみ）・ローカルMCP（scripts/mcp_server.py）の3経路から選べる。いずれも実体は同じスクリプトに委譲される。
...
- トークンは発行・再発行の瞬間にチャットへ一度だけ表示し、平文でファイルやリポジトリに永続保存しない（閲覧者側ではCookieに保持される点は既知の設計）
...
- `infra/cloudfront-function/edge-gate.js`: トークン照合・無効化判定・コメントPUT検査・一覧書き換えを行うエッジ関数（KVS読み取りは1回/リクエスト）
- `scripts/ds.sh`: 統一CLI: list／deploy／export／rotate／disable／console／update-function
...
- `scripts/console_server.py`: Web管理コンソール（localhostのみ・Host検証＋乱数トークンでCSRF防御）
