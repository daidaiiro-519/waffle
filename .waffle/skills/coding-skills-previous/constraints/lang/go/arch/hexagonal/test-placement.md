...
| `internal/app` | 同じパッケージの `_test.go` | 業務操作の振る舞い | 二次のポートの偽物を差し込んで呼ぶ |
| 二次のポート（interface） | 置かない | ─ | 宣言だけなので、確かめる振る舞いが無い |
| `internal/adapter/inbound` | 外部パッケージの `_test.go` | 外部の形式から業務操作へ渡るまで | 公開する識別子だけを呼ぶ |
| `internal/adapter/outbound` | 外部パッケージの `_test.go` | 二次のポートの実装が契約どおりに振る舞うか | 公開する識別子だけを呼ぶ |

## 偽物の置き場所

| 偽物 | 置き場所 | 作り方 | 置いてはならない場所 |
|---|---|---|---|
| 二次のポートの偽物 | `internal/app` の `_test.go` | interface を満たす最小の構造体 | `adapter`。外側の実装とテストの偽物が混ざる |
| 入力の見本 | `testdata/` | 実物の形式そのまま | 内側。内側が外部の形式を知ることになる |
| 外部サービスの偽物 | 使わない | ─ | 二次のポートの偽物で足りる |
...
| GH-TP-03 | 二次のポートの偽物は `app` の中に置き、`adapter` へ置かない | 必須 | 内側のテスト |
...
### GH-TP-03　二次のポートの偽物は `app` の中に置き、`adapter` へ置かない
...
| 検証方法 | 当たる命令が無い。`adapter` の中に、二次のポートを満たす型の定義が無いかを見る |
| 例外 | なし |
| 既存コードへの適用 | 改修時に是正 |
