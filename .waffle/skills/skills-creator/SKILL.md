...
references/skill-template.mdを読み込む。各{{...}}プレースホルダーは、そこに書かれた指示を読めばそのまま埋められる自己完結の形式になっているため、別途定義票を参照する必要はない。ヒアリング内容に従って全プレースホルダーを埋め、{{スコープ}}/skills/{{スキル名}}/SKILL.mdとして保存する。
