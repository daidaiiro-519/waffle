...
| 作業は UDDループ（調べる→決める→引き継ぐ→作る）を通す。どこまで踏むかはその都度判断する——新規capability・挙動変更・schema変更を伴うなら4段とも踏み、単一フィールド修正・機械的な文言変換・調査単体なら決めることが無いので段は薄くなる。 | 経路を2つ用意して先に選ばせると、『どちらの道か』という判断が本来の判断の前に増える。しかも一方の名前が付けられなかった——自然な名前が付かないのは、分ける必要が無いことの合図である。1本のループに寄せても判断は無くならず、spec合意前の実装（既知の再発パターン）と、軽微な修正への過剰な儀式化のどちらへ振れるかを、その都度測ることになる。 | 境界事例（例：複数フィールドにまたがるが挙動は変わらない修正）は judgmentTasks の基準に従う。迷ったら『決めることがあるか』を先に問う——決めることがあるなら、決める段と引き継ぐ段を省かない。 |
...
UDDループ（調べる→決める→引き継ぐ→作る）を1本の道として通し、作業ごとにどこまで踏むかを判断するOrchestrator。決めることがある作業で、決める段と引き継ぐ段を省かないことに責任を持つ。構造の検証・生成・描画はWaffleが機械的に担い、判断はアドバイザーSkillが敵対的に確かめる。
...
| 作業が UDDループ のどの段まで踏むべきか自明でないとき | 「振る舞いが変わるか」「将来同種の判断が繰り返されるか」のいずれかが Yes なら4段とも踏む。両方 No なら、決めることが無いので決める段と引き継ぐ段は薄くてよい。 | レーンの定義だけでは境界事例を割り切れないため、最終判断はOrchestrator自身に残す |
...
|---|---|---|---|---|
| skill-router | before | category | 作業を始める前に、まずskill-routerへ問い合わせ、配線表（`skill-wiring.json`）にその作業の行があるかを確認する。行があれば「何を頼むか」と「呼ぶSkill」を受け取り、続く委譲パターンへの入力として渡す。行が無ければ、その作業は1つのSkillで終えられるものとして進む | 作業側のSkillと呼ばれる側のSkillが互いを呼ぶ構造を避け、判断の窓口をOrchestrator側に置くため（skill-router自身の設計原則）。呼ぶタイミング（作業の前に材料を集める・作業のあとに十分かを確かめるのいずれか、または両方）はOrchestrator自身が判断する |
| advisor（ddd-advisor/tech-lead-advisor/ux-advisor/qa-advisor等） | after | category | skill-routerが返した「呼ぶSkill」に従い、敵対的検証を依頼する各advisor（同じadvisorへの敵対的検証フェーズ再検証を含む）ごとに、AgentSchemaのgoal-dispatch構造（目的・役割・読み込むSkill・タスク・成果物・受け入れ基準）で内容を組み立て、Agentツールでwaffleサブエージェントとして並列に呼び出す（汎用のエージェントへ要約した指示を渡す代行はしない。受け手が読み込むべきSkillのSKILL.md本文の具体的な手順、特にresponseTypesが指すテンプレートファイルの読み込みが遂行される保証がないため）（1体ずつ直列に呼ばない。複数advisorへの依頼は1回のメッセージで同時に発行する）。敵対的検証の対象が1件のみで他advisorとの突き合わせが不要な場合は単体呼び出しでよい。全員の結果が揃ったら、各回答がresponseTypesが指すテンプレートの見出し構造を実際に踏襲しているかを確認してから、template-skill-adversarial-verification.mdの形式で統合する（テンプレート構造を踏まない回答は、内容の質も未検証とみなし再実行する） | advisorはWaffleが所有・出荷する成果物であり、単一advisorの一発出力を無検証で確定させない運用はWaffle自身のOrchestratorの責務。advisorが互いを呼ぶのではなくOrchestrator側が組合せを判断することで、Skill/advisor間のテキストベース疎結合原則を保てる。並列dispatchにするのは、直列に1体ずつ呼ぶと後段のadvisorが前段の結論に引きずられ、独立した意見として機能しなくなるため（診断的差し戻しの前提となる複数視点の独立性を保つ）。呼び出し手段・出力検証を明示するのは、要約した指示による代行dispatchでは受け手がテンプレート等の具体的な手順を遂行する保証がなく、実際に本セッションでテンプレート未遵守の回答を無検証で確定させかけた再発防止のため |
