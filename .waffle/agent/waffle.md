...
| skill-routerの概念的な手順のうち、ルーティング表の反映をWaffleコマンドへ対応させる（Skill自身にはWaffle固有の語彙を持ち込まない。対応関係はここに集約する） | 「ルーティング表の該当行を追加・修正・削除する」= `content.routingTable.items`を query で現在値取得→組み立て→`waffle scaffold fill --path .waffle/documents/skills/skill-router.json`で丸ごと置き換え→`waffle validate`→`waffle render` |
...
|---|---|---|---|---|
| skill-router | before | category | Spec-authoring/Handoff-authoring/Implementationのいずれかを使う前に、まずskill-routerへ問い合わせ、routingTableが示すadvisorとの組み合わせ（WHO）を確認する（Investigationはrouting表にエントリが無く単独で完結するため対象外）。返ってきたcombinedSkillsの一覧を、続くadvisor委譲パターンへの入力として渡す | 実務Skillとadvisor Skillが互いを呼ぶ構造を避け、組み合わせ判断の一次窓口をOrchestrator側に置くため（skill-router自身の設計原則）。呼ぶタイミング（執筆前の判断材料収集・執筆後の十分性チェックのいずれか、または両方）はOrchestrator自身が判断する |
| advisor（ddd-advisor/tech-lead-advisor/ux-advisor/qa-advisor等） | after | category | skill-routerが返したcombinedSkillsに従い、敵対的検証を依頼する各advisor（同じadvisorへの敵対的検証フェーズ再検証を含む）ごとに、AgentSchemaのgoal-dispatch構造（目的・役割・読み込むSkill・タスク・成果物・受け入れ基準）で内容を組み立て、Agentツールでwaffleサブエージェントとして並列に呼び出す（汎用のエージェントへ要約した指示を渡す代行はしない。受け手が読み込むべきSkillのSKILL.md本文の具体的な手順、特にresponseTypesが指すテンプレートファイルの読み込みが遂行される保証がないため）（1体ずつ直列に呼ばない。複数advisorへの依頼は1回のメッセージで同時に発行する）。敵対的検証の対象が1件のみで他advisorとの突き合わせが不要な場合は単体呼び出しでよい。全員の結果が揃ったら、各回答がresponseTypesが指すテンプレートの見出し構造を実際に踏襲しているかを確認してから、template-skill-adversarial-verification.mdの形式で統合する（テンプレート構造を踏まない回答は、内容の質も未検証とみなし再実行する） | advisorはWaffleが所有・出荷する成果物であり、単一advisorの一発出力を無検証で確定させない運用はWaffle自身のOrchestratorの責務。advisorが互いを呼ぶのではなくOrchestrator側が組合せを判断することで、Skill/advisor間のテキストベース疎結合原則を保てる。並列dispatchにするのは、直列に1体ずつ呼ぶと後段のadvisorが前段の結論に引きずられ、独立した意見として機能しなくなるため（診断的差し戻しの前提となる複数視点の独立性を保つ）。呼び出し手段・出力検証を明示するのは、要約した指示による代行dispatchでは受け手がテンプレート等の具体的な手順を遂行する保証がなく、実際に本セッションでテンプレート未遵守の回答を無検証で確定させかけた再発防止のため |
