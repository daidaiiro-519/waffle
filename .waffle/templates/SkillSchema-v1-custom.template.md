...
| `{{処理対象と成果物.処理対象}}` | このSkillが受け取る処理対象を自然言語で記述してください。何を・どのような状態のものを受け取るかを明示してください。（JSON上のフィールド: content.processingTarget.target） |
| `{{処理対象と成果物.成果物}}` | このSkillが生み出す成果物を自然言語で記述してください。何を・どのような形式で出力するかを明示してください。（JSON上のフィールド: content.processingTarget.artifact） |
...
| `{{出力形式}}` | 実行結果・報告の形式を指定してください（例:「チェック項目ごとに✓/✗と理由を箇条書きで報告する」）。無ければこのブロック自体を省略してください。（任意ブロック：無ければ出力形式ブロック自体を省略してよい）（JSON上のフィールド: content.outputFormat.text） |
| `{{ガードレール.項目1}}` | このSkillを実行する際に必ず守るべき制約・禁止事項・前提条件を列挙してください。（配列。この形式の行を必要な数だけ繰り返す）（JSON上のフィールド: content.guardrails.items[]） |
| `{{参照.項目1.パス}}` | 参照先のファイルパスまたはスキーマ参照を記述してください。例: .waffle/schemas/SkillSchema/v1.json（配列。この形式の行を必要な数だけ繰り返す）（JSON上のフィールド: content.references.items[].path） |
| `{{参照.項目1.説明}}` | このリソースが何を提供するかを1文で記述してください。（配列。この形式の行を必要な数だけ繰り返す）（JSON上のフィールド: content.references.items[].description） |
