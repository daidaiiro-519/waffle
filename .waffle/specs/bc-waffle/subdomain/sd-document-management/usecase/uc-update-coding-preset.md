...
### 指定されたプリセットが存在しないときのときPRESET_NOT_FOUND

| 分類 | 観点 |
|---|---|
| 異常系 | エラー：指定されたプリセットが存在しないとき |

```gherkin
Scenario: 指定されたプリセットが存在しないときのときPRESET_NOT_FOUND
  Given 指定されたプリセットが存在しないとき状況
  When 本ユースケースを実行する
  Then PRESET_NOT_FOUND エラーが返る
```

### 出どころとして指定された規約が存在しないときのときINVALID_PATH

| 分類 | 観点 |
|---|---|
| 異常系 | エラー：出どころとして指定された規約が存在しないとき |

```gherkin
Scenario: 出どころとして指定された規約が存在しないときのときINVALID_PATH
  Given 出どころとして指定された規約が存在しないとき状況
  When 本ユースケースを実行する
