...
id: "sd-flow-gate"
type: "subdomain"
title: "開発フローのフェーズ遷移を機械的に判定するサブドメイン：sd-flow-gate"
description: "ブレスト→spec→TDD→検証→レビューという開発フローの各フェーズ遷移について、進んでよいか(ready)・まだ足りないか(blocked)・機械的に判別できず人間の判断が要るか(needs_human)を判定し続ける業務領域。sd-reconciliationが「既存の成果物間の構造的整合性」を検知するのに対し、こちらは「プロセスが次フェーズへ進める状態にあるか」というプロセス状態そのものを対象にする。"
