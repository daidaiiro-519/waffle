...
- どの層にも属さない実装が列挙されている
- 実装どうしの閉じた依存（循環）が、関わる実装の一覧とともに列挙されている
...
| When 実装どうしが互いを辿って元へ戻る依存を持つとき、層の突き合わせは それを1つの循環として報告する shall |
| When 同じ循環が複数の始点から辿れるとき、層の突き合わせは それを1件だけ報告する shall |
...
### 互いを参照し合う実装は循環として報告する

| 分類 | 観点 |
|---|---|
| 異常系 | 依存の健全性：向きの規則では捕まらない、閉じた依存を検出する |
...
| 分類 | 観点 |
|---|---|
| 境界値 | 報告の一意性：見る順序という本質と無関係な要因で件数が変わらない |
...
| 分類 | 観点 |
|---|---|
| 正常系 | 依存の健全性：閉じた依存が無い状態を差分として出さない |
...
### 指定された規約が見つからないときのときARCHITECTURE_REF_NOT_FOUND

| 分類 | 観点 |
|---|---|
| 異常系 | エラー：指定された規約が見つからないとき |

```gherkin
Scenario: 指定された規約が見つからないときのときARCHITECTURE_REF_NOT_FOUND
  Given 指定された規約が見つからないとき状況
  When 本ユースケースを実行する
  Then ARCHITECTURE_REF_NOT_FOUND エラーが返る
```

### 指定された規約が層の置き場所を決めるのに足りる宣言を持たないときのときARCHITECTURE_REF_UNRESOLVED

| 分類 | 観点 |
|---|---|
| 異常系 | エラー：指定された規約が層の置き場所を決めるのに足りる宣言を持たないとき |

```gherkin
Scenario: 指定された規約が層の置き場所を決めるのに足りる宣言を持たないときのときARCHITECTURE_REF_UNRESOLVED
  Given 指定された規約が層の置き場所を決めるのに足りる宣言を持たないとき状況
  When 本ユースケースを実行する
  Then ARCHITECTURE_REF_UNRESOLVED エラーが返る
```
