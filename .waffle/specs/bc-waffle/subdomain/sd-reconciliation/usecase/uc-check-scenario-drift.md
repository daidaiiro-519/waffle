...
description: "仕様が宣言したシナリオに対応するテストが実在するか、そしてどのシナリオにも対応しないテストが残っていないかを機械的に確かめる。"
tags: ["context:waffle"]
...
- 仕様が宣言したシナリオに対応するテストが実在するか、そしてどのシナリオにも対応しないテストが残っていないかを機械的に確かめる。
...
- 全体を検査したときの返り値は、missing_test_file（宣言はあるが突き合わせる相手のテストファイルが無いシナリオブロック）と、missing_placement（規約が置き場所を宣言していないため、突き合わせの対象にさえならなかったシナリオ種別）と、results（相手が居た組それぞれの、1組検査と同じ6フィールドに文書の識別子を添えたもの）を持つ
...
- 対象となるシナリオブロックは、規約が宣言したシナリオ種別と配置の対応から機械的に絞り込む。どの配置にも当てはまらない場合は絞り込まず、宣言されている全種別を対象にする（ケースバイケースの判定はしない）。配置の具体的な場所は規約が持つので、ここへ写さない
...
- 文書コメントが関数の内側に置かれる言語では、コメントとテストの対応は構造的に決まる。関数の直前に置かれる言語では直前の兄弟であることによって決まり、間に別のコメントや文が挟まると対応しなくなる（対応しない場合は孤立と未実装の両方で報告されるため、黙って通ることはない）
...
| While テストの置かれた場所が、規約が宣言したいずれかの配置に当てはまるとき、システムはその配置に対応づくシナリオ種別だけを対象に突き合わせを行う shall。 |
| While テストの置かれた場所が、規約が宣言したどの配置にも当てはまらないとき、システムは宣言されている全てのシナリオ種別を対象に突き合わせを行う shall（絞り込めないときに黙って一部だけを見る形にしない）。 |
| While 規約の配置の宣言に、対象のシナリオ種別まで指定した行があるとき、システムはその行を、種別を指定していない行より優先する shall。 |
...
| `MISSING_DECLARATION` | - テストファイルの名前を組み立てる宣言が、対象のスタックの規約に無い |
...
| 分類 | 観点 |
|---|---|
| 境界値 | 境界: 同じ層・同じテスト種別でも、種別ごとに別の場所へ置かれる言語がある |
...
### 対象のspec.jsonまたはテストファイルが存在しないときのときINVALID_PATH

| 分類 | 観点 |
|---|---|
| 異常系 | エラー：対象のspec.jsonまたはテストファイルが存在しないとき |

```gherkin
Scenario: 対象のspec.jsonまたはテストファイルが存在しないときのときINVALID_PATH
  Given 対象のspec.jsonまたはテストファイルが存在しないとき状況
  When 本ユースケースを実行する
  Then INVALID_PATH エラーが返る
```

### 名前を組み立てる宣言が規約に無いときのときMISSING_DECLARATION

| 分類 | 観点 |
|---|---|
| 異常系 | エラー：名前を組み立てる宣言が規約に無いとき |

```gherkin
Scenario: 名前を組み立てる宣言が規約に無いときのときMISSING_DECLARATION
  Given 名前を組み立てる宣言が規約に無いとき状況
  When 本ユースケースを実行する
  Then MISSING_DECLARATION エラーが返る
```
...
| 分類 | 観点 |
|---|---|
| 正常系 | 計算整合: 名前の綴りの規則が検査の側に無いことの証明。この一点がこの変更の核心 |
...
| 分類 | 観点 |
|---|---|
| 境界値 | 計算整合: 宣言を読み捨てて元の綴りを内部で作り直しても、この筋書きだけが赤くなる |
...
| 分類 | 観点 |
|---|---|
| 境界値 | 境界: 黙って対象から外すと、宣言の欠落と検査に通ったことが同じ見た目になる |
