...
Agent(subagent)集約の実インスタンスがskillPreloadsで参照するSkillが、実在するかを機械的に検証する。Waffleのschema単体からは検出できないSubagent文書↔Skill文書間の参照整合性のドリフト。
...
- **意図**: Subagentがskillsとしてプリロード指定しているSkillが、実在する状態であるかを確認したい

---

## 存在意義

AgentSchemaとSkillSchemaはそれぞれ独立に検証されるため、Subagentが参照するSkillの実在性は、どちらのschema単体からも検出できない盲点になっている。この検知が無ければ、実在しないSkillをプリロード指定したまま気づかれず、実行時に初めて（あるいは気づかれないまま）機能しない。
...
sequenceDiagram
    ...
    AgentSkillTree-->>Orchestrator: missing_skillsのオブジェクト配列を返す
```
...
- 返り値は次の1フィールドを持つ: missing_skills（参照先Skill documentが実在しないAgent-skillIdの組）
...
- missing_skillsが空配列であれば、全subagentのskillPreloads参照が実在する（正常系）
...
- While 全subagentのskillPreloads参照が実在するとき、システムはmissing_skillsを空配列で返す shall。
...
### 全subagentの参照が実在するとき差分なしと判定する

| 分類 | 観点 |
|---|---|
| 正常系 | 整合：全skillPreloads参照が実在するSkillを指しているとき正常系（空配列） |

```gherkin
Scenario: 全subagentの参照が実在するとき差分なしと判定する
