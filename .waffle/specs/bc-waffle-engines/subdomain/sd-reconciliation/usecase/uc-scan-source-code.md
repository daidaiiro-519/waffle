...
- DocstringSchema（本ユースケースというドメインサービスの戻り値の型。どの集約にも属さず、どこにも永続化されない）
...
- When 対象パスと kind が与えられたとき、エンジンは各公開要素を {path, kind, elementKind, name, hasDocstring, signatureParams, summary, body, args, returns, raises, attributes} の構造で返す shall。
