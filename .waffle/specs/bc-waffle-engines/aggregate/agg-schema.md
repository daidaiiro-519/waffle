...
| 新schemaのx-migration宣言は常にMigrationMetaSchemaの閉じた語彙(rename/default/value-map/discriminator-remap/ai-infer)にのみ従う | schema | 移行がengineによって機械的に解釈可能であることを保つ |
| Documentのschemaが指しうる型は常にx-schema-statusを宣言する | schema | 第4のドリフト検知(check_schema_version_drift.py)がバージョンの状態を判定できることを保証する |
| 解決できないschemaRefを対象にした操作は、いかなる operation・command からも常に INVALID_SCHEMA_REF として拒否される | guard | scaffold/migrateDocuments 等、複数 usecase が同じ「schemaRef を解決できない」状況に個別に対応していた（重複）。Schema 集約が扱う全ての操作に横断的に適用される不変条件として一箇所で保証する |
...
### rename宣言は_from_を要求する

| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: x-migrationはMigrationMetaSchemaの閉じた語彙にのみ従う(rename) |
...
### default宣言は_value_を要求する

| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: x-migrationはMigrationMetaSchemaの閉じた語彙にのみ従う(default) |
...
### value_map宣言は_from_と_mapping_を要求する

| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: x-migrationはMigrationMetaSchemaの閉じた語彙にのみ従う(value-map) |
...
### discriminator_remap宣言は_rules_を要求する

| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: x-migrationはMigrationMetaSchemaの閉じた語彙にのみ従う(discriminator-remap) |
...
### ai_infer宣言は_prompt_を要求する

| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: x-migrationはMigrationMetaSchemaの閉じた語彙にのみ従う(ai-infer) |
...
| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: x-migrationの閉じた語彙にない種別は拒否される |
...
| 分類 | 観点 |
|---|---|
| 正常系 | MigrationMetaSchema自体が正しくロードできることの確認 |
...
### 解決できないschemaRefはINVALID_SCHEMA_REFとして拒否される

| 分類 | 観点 |
|---|---|
| 異常系 | 不変条件: 解決できないschemaRefへの操作はINVALID_SCHEMA_REFとして拒否される |
