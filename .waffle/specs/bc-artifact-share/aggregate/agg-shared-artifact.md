...
| 表す値 | 振る舞い |
|---|---|
| 公開された表示物を一意に指す短いID | 不変。公開時に発行され、以後どの操作でも変わらない。差し替えても再公開しても同じ値を保つ。 |
...
| IDは、公開してから取り消すまで常に変わらない | schema | - 一度渡した居場所が変わらないことが、差し替えても同じ場所で見続けられるという体験の前提になるため。 |
...
表示物を受け取って公開し、IDと合鍵を発行する。中身をそのまま保持し、書き換えない。
...
中身だけを新しいものへ丸ごと置き換える。ID・合鍵・それまでのコメントを保つ。
...
新しい合鍵を発行し、それまでの合鍵を使えなくする。IDは変えない。
