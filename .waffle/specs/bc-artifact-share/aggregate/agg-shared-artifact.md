...
| 表す値 | 振る舞い |
|---|---|
| 共有アーティファクトに添えられた識別子・種別・題名・要約・分類の目印 | 不変。中身に書かれていれば取り出し、書かれていなければ題名だけを人が与える。種別と分類の目印は保持するだけで、値の意味を解釈しない。source は目印の出どころ（中身から取り出したか、人が与えたか）を表す。 |
...
| 1つの共有アーティファクトが同時に入れるプロジェクトは、常に3件を超えない | guard | - 閲覧ゲートは所属を、書き手と読み手が同じ数を知っている前提の投影から読む。書き手が黙って超えると、超えた分は閲覧ゲートから見えないまま所属したことになり、投稿者には成功が返って閲覧者だけが開けない。<br>- 所属の正は共有アーティファクトの側が持つ。プロジェクトの側の一覧は、閲覧者へ見せるために組み立て直せる投影であり、失敗しても作り直せる。 |
...
この共有アーティファクトをプロジェクトへ加える。人が明示的に選んだ結果としてのみ実行される。同時に入れるプロジェクトは3件を超えない。
...
この共有アーティファクトをプロジェクトから外す。共有アーティファクト自体は失われず、それぞれの閲覧トークンで引き続き開ける。
