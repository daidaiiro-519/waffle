...
| 操作 | 概要 |
|---|---|
| `assign` | 共有アーティファクトをプロジェクトへ加える。以後はそのプロジェクトの閲覧トークンでも開けるようになる。 |
| `unassign` | 共有アーティファクトをプロジェクトから外す。共有アーティファクト自体と、それ自身の閲覧トークンは失われない。 |
...
| `TOO_MANY_PROJECTS` | - 入れられる単位の数が上限に達している |
...
| 分類 | 観点 |
|---|---|
| 境界値 | 境界値: 上限は書き手の側だけが守る。閲覧の面は先頭から決まった数しか見ないため、超過を許すと投稿者には成功が返り、閲覧者だけが開けない状態になる |
