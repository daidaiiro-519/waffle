...
- 閲覧トークンを持つ閲覧者が、共有アーティファクトに反応を残す操作。名乗るだけでよく、あらかじめ登録されている必要はない。
...
- これが無いと、見てもらった相手の反応を別の手立てで集めることになり、どの指摘がどの共有アーティファクトに対するものかが散らばる。
...
- 同じ共有アーティファクトに付いた反応を返信先として指せること
- 別の共有アーティファクトに付いた反応を返信先として指したとき、拒むこと
- 停止している共有アーティファクトへ反応を残そうとしたとき、拒むこと
...
| コード | 条件 |
|---|---|
| `ARTIFACT_NOT_VIEWABLE` | - 対象の共有アーティファクトが停止している<br>- 閲覧トークンを持っていない |
| `AUTHOR_REQUIRED` | - 名乗りが与えられていない |
| `BODY_REQUIRED` | - 本文が空である |
| `INVALID_PARENT` | - 返信先として指された反応が、別の共有アーティファクトに付いている |
...
```gherkin
Given 共有アーティファクトAに反応が1件ある
When その反応を返信先に指して新しい反応を残す
Then 返信として結び付いて残る
```

### 別の共有アーティファクトの反応へは返信できない
...
When 共有アーティファクトAへの反応として、共有アーティファクトBの反応を返信先に指す
