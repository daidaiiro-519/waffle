...
- プロジェクトの閲覧トークンを再発行したり、開けない状態にしたり、再び開けるようにする操作。渡した相手を絞り直したいときに使う。
- これらは閲覧トークンを渡した相手全員に及ぶため、共有のプロジェクトでも持ち主と管理者だけが行える。
...
| 操作 | 概要 |
|---|---|
| `reissue` | 新しい閲覧トークンを発行し、それまでのものを使えなくする。 |
| `suspend` | このプロジェクトの閲覧トークンでは何も開けない状態にする。 |
| `resume` | 再び開ける状態に戻す。閲覧トークンは必ず新しくなる。 |
...
sequenceDiagram
    actor 投稿者
    投稿者->>プロジェクト: reissueViewToken / suspend / resume
    プロジェクト->>プロジェクト: 持ち主か管理者かを確かめる
    プロジェクト->>プロジェクト: 閲覧トークンを差し替えるか、開ける状態を切り替える
    プロジェクト-->>投稿者: 新しい閲覧トークン、または切り替えた旨
```
...
- 再発行したときは、新しい閲覧トークンが発行され、それまでのものが使えなくなっている
...
| `NOT_ACTIVE` | - 公開停止しているプロジェクトを、再発行または公開停止しようとした |
...
| 分類 | 観点 |
|---|---|
| 正常系 | 状態遷移: 渡した範囲を絞り直す手段として働くことを確かめる |

```gherkin
Given 相手がプロジェクトの閲覧トークンを持っている
When Xが再発行を求める
Then 新しい閲覧トークンが返る
And 相手はそれまでの閲覧トークンで開けない
And 入っている共有アーティファクトはAとBのままである
```
...
| 分類 | 観点 |
|---|---|
| 正常系 | 状態遷移: 止めた意図が、再開によって失われないことを確かめる |

```gherkin
Given プロジェクトが公開停止されている
When Xが再開を求める
Then 開ける状態に戻る
And 新しい閲覧トークンが返る
And 止める前の閲覧トークンでは開けない
```
